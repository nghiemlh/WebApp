	...
	public class FunctionController : ApiControllerBase
	{
		...
		public HttpResponseMessage GetAllHierachy(HttpRequestMessage request)
		{
			return CreateHttpResponse(request, () =>
			{
				...
				var parents = modelVm.Where(x => x.ParentId == null).OrderBy(x => x.DisplayOrder);
				foreach (var parent in parents)
				{
					parent.ChildFunctions = modelVm.Where(x => x.ParentId == parent.Id).OrderBy(x => x.DisplayOrder).ToList();
					if (parent.ChildFunctions.Count > 0)
					{
						foreach (var subParent in parent.ChildFunctions)
						{
							var subParentItems = modelVm.Where(x => x.ParentId == subParent.Id).OrderBy(x => x.DisplayOrder).ToList();
							if (subParentItems.Count > 0)
							{
								subParent.ChildFunctions = modelVm.Where(x => x.ParentId == subParent.Id).OrderBy(x => x.DisplayOrder).ToList();
							}
						}
					}
				}
		...
		public HttpResponseMessage GetAll(HttpRequestMessage request, string filter = "")
		{
			...
			{
				HttpResponseMessage response = null;
				var model = _functionService.GetAllStatus(filter).OrderBy(x => x.DisplayOrder);
				IEnumerable<FunctionViewModel> modelVm = Mapper.Map<IEnumerable<Function>, IEnumerable<FunctionViewModel>>(model);
