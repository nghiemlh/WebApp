...
namespace WebApp.Web.Controllers
{
    ...
    public class AnnouncementController : ApiControllerBase
    {
        ...
        public HttpResponseMessage Create(HttpRequestMessage request, AnnouncementViewModel announcementVm)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
					...
					newAnnoun.CreatedBy = User.Identity.Name;
                    ...
                    //push notification
                    WebAppHub.PushToAllUsers(Mapper.Map<Announcement, AnnouncementViewModel>(announ), null);
		...
		[Route("update")]
		[HttpPut]
		public HttpResponseMessage Update(HttpRequestMessage request, AnnouncementViewModel announcementVm)
		{
			return CreateHttpResponse(request, () =>
			{
				HttpResponseMessage response = null;
				...
				{
					var dbAnnouncement = _announcementService.GetById(announcementVm.Id);

					dbAnnouncement.Content = announcementVm.Content;
					dbAnnouncement.Status = announcementVm.Status;
					dbAnnouncement.Title = announcementVm.Title;
					dbAnnouncement.UpdatedDate = DateTime.Now;
					dbAnnouncement.UpdatedBy = User.Identity.Name;
					_announcementService.Update(dbAnnouncement);
					_announcementService.Save();

					var responseData = Mapper.Map<Announcement, AnnouncementViewModel>(dbAnnouncement);
